...
namespace PsychsonGUI
{
    public class PhisonDevice
    {
        ...
        public PhisonDevice(char driveLetter, RichTextBox tracebox)
        {
        ...
        public byte[] RequestInfo(byte[] request, int expected)
        {
            var data = SendCommand(request, expected);
            byte[] ret = null;
            if (data != null)
                ret = data.Take(expected).ToArray();
            return ret;
        }
        ...
        public void TransferFile(byte[] data, ToolStripProgressBar pb = null)
        {
            TransferFile(data, 0x03, 0x02, pb);
        }

        public void TransferFile(byte[] data, byte header, byte body, ToolStripProgressBar pb = null)
        {
        ...
        private static void Log(byte[] cmd, byte[] data, byte[] recv)
        {
            if (cmd != null)
                box.AppendText("USB CMD: 0x" + cmd.Length.ToString("X4") + " bytes\n" + HexDump(cmd) + "\n");
            if (data != null)
                box.AppendText("USB DATA: " + data.Length.ToString("X4") + " bytes\n" + HexDump(data) + "\n");
            if (recv != null)
                box.AppendText("USB RECV: " + recv.Length.ToString("X4") + " bytes\n\n" + HexDump(recv) + "\n");
